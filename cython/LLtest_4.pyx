...
def plotdat(arr,pflag,nmax): # plot的設定 2號
    ...
    if pflag==1: # colour the arrows according to energy
        ...
        for i in range(nmax): # 行
            for j in range(nmax): # 列
                cols[i,j] = one_energy(arr,i,j,nmax)
        #存完nmax * nmax個分子能量
        norm = plt.Normalize(cols.min(), cols.max())
    elif pflag==2: # colour the arrows according to angle
        mpl.rc('image', cmap='hsv')
        cols = np.asarray(arr)%np.pi # 正規化，因為液晶分子有頭尾對稱性 所以圓和半圓範圍對應的值是等價的 # np.array-> 數值化才能%
...
# 算角度先，然後再依照公式計算en，每個都算一遍 但這邊他有變換一下公式
    ang = point-arr[ixp,iy]
    cos_value = cos(ang)*cos(ang)
    en += 0.5*(1.0 - 3.0*cos_value)
    ang = point-arr[ixm,iy]
    cos_value = cos(ang)
    en += 0.5*(1.0 - 3.0*cos_value)
    ang = point-arr[ix,iyp]
    cos_value = cos(ang)
    en += 0.5*(1.0 - 3.0*cos_value)
    ang = point-arr[ix,iym]
    cos_value = cos(ang)
    en += 0.5*(1.0 - 3.0*cos_value)
    return en

cdef double MC_step(double[:,:]arr,float Ts,int nmax): #一次 mc step 代表 轉動一個分子看全部會如何
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
...
cpdef main(str program, int nsteps, int nmax, float temp, int pflag): # 第一處理function
    """
    Arguments:
