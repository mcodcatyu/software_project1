...
def all_energy(double[:,:] arr,int nmax, int num_threads): 
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
      nmax (int) = side length of square lattice.
...
cdef double MC_step(double[:,:]arr,float Ts,int nmax): 
    """
    Arguments:
	  arr (float(nmax,nmax)) = array that contains lattice data;
	  Ts (float) = reduced temperature (range 0 to 2);
...
cpdef main(str program, int nsteps, int nmax, float temp, int pflag, int num_threads): 
    """
    Arguments:
